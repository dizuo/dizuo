...
QEasingCurve g_Curve(QEasingCurve::InCirc);
...
int fakeSphere(int i, int j)
{
	...
	float value = r/Rad;	//[0.0, 1.0]
	//float result = 1/(1+value+value*value);
	float result = powf(0.1, value);			// 镜面高光，底数越小衰减越快

	if( r<Rad )
		return (int)(result*255);
		//return (int)( (1-r/Rad)*255 );		//半径方向线性变化
		//return (int)( 255*(1-g_Curve.valueForProgress(value)) );
