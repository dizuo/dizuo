...
---------------------------------------------------------------------*/
tVertex	ConvexHull3D::MakeNullVertex( void )
{
	tVertex  v;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::ReadVertices( const char* fileName )
{
	tVertex  v;
	char chv;
	float    x, y, z;
	...
	while ( fscanf (pFile, "%f %f %f", &x, &y, &z ) != EOF )  {
	//while ( fscanf (pFile, "%c %f %f %f", &chv, &x, &y, &z ) != EOF ) { 
		v = MakeNullVertex();
		v->v[X] = x * 100;
		v->v[Y] = y * 100;
		v->v[Z] = z * 100;
...
int ConvexHull3D::GetNewVerticeIndex(int *pIdx, int nV, int cmpData)
{
	int i(0);
	for (; i<nV; i++)
	{
...
void	ConvexHull3D::Print( void )
{
	...
	//	if(( a[0] * b[1] - a[1] * b[0] ) >= 0 )	// ( a.cross(b) ).z() > 0
	//	{
	//		printf("%%%% vnums:  %d  %d  %d\n", 
	//			f->vertex[0]->vnum, 
	//			f->vertex[1]->vnum, 
	...
	do {
		int idx0 = GetNewVerticeIndex(pIndex, V, f->vertex[0]->vnum);
		int idx1 = GetNewVerticeIndex(pIndex, V, f->vertex[1]->vnum);
		int idx2 = GetNewVerticeIndex(pIndex, V, f->vertex[2]->vnum);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void ConvexHull3D::PrintToFile(const char* fileName)
{
	FILE * pFile;
	...
	f = faces;
	/* prints a list of all faces */
	fprintf( pFile, "%%%% List of all faces: \n");
	fprintf( pFile, "%%%%\tv0\tv1\tv2\t(vertex indices)\n");
...
---------------------------------------------------------------------*/
void    ConvexHull3D::SubVec( int a[3], int b[3], int c[3])
{
	int  i;
...
are cleaned up. 
---------------------------------------------------------------------*/
void    ConvexHull3D::DoubleTriangle( void )
{
	tVertex  v0, v1, v2, v3;
	tFace    f0, f1 = NULL;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::ConstructHull( void )
{
	tVertex  v, vnext;
...
---------------------------------------------------------------------*/
bool 	ConvexHull3D::AddOne( tVertex p )
{
	tFace  f; 
	tEdge  e, temp;
	int 	  vol;
...
---------------------------------------------------------------------*/
int  ConvexHull3D::VolumeSign( tFace f, tVertex p )
{
	double  vol;
	int     voli;
...
---------------------------------------------------------------------*/
int  ConvexHull3D::Volumei( tFace f, tVertex p )
{
	int  vol;
...
/*-------------------------------------------------------------------*/
void	ConvexHull3D::PrintPoint( tVertex p )
{
	int	i;
...
the new face.
---------------------------------------------------------------------*/
tFace	ConvexHull3D::MakeConeFace( tEdge e, tVertex p )
{
	tEdge  new_edge[2];
	tFace  new_face;
	int 	  i, j;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::MakeCcw( tFace f, tEdge e, tVertex p )
{
...
---------------------------------------------------------------------*/
tEdge 	ConvexHull3D::MakeNullEdge( void )
{
	tEdge  e;
...
to the empty cell.
---------------------------------------------------------------------*/
tFace 	ConvexHull3D::MakeNullFace( void )
{
	tFace  f;
	int    i;
...
---------------------------------------------------------------------*/
tFace   ConvexHull3D::MakeFace( tVertex v0, tVertex v1, tVertex v2, tFace fold )
{
	tFace  f;
	tEdge  e0, e1, e2;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::CleanUp( tVertex *pvnext )
{
	CleanEdges();
	CleanFaces();
...
---------------------------------------------------------------------*/
void	ConvexHull3D::CleanVertices( tVertex *pvnext )
{
	tEdge    e;
	tVertex  v, t;
...
---------------------------------------------------------------------*/
bool	ConvexHull3D::Collinear( tVertex a, tVertex b, tVertex c )
{
	return 
...
---------------------------------------------------------------------*/
void	ConvexHull3D::Consistency( void )
{
	register tEdge  e;
	register int    i, j;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::Convexity( void )
{
	register tFace    f;
	register tVertex  v;
	int               vol;
...
---------------------------------------------------------------------*/
void	ConvexHull3D::CheckEuler( int V, int E, int F )
{
	if ( check )
...
=====================================================================*/
/*-------------------------------------------------------------------*/
void	ConvexHull3D::PrintOut( tVertex v )
{
	fprintf( stderr, "\nHead vertex %d = %6x :\n", v->vnum, v );
	PrintVertices();
	PrintEdges();
...
/*-------------------------------------------------------------------*/
void	ConvexHull3D::PrintVertices( void )
{
	tVertex  temp;
...
/*-------------------------------------------------------------------*/
void	ConvexHull3D::PrintEdges( void )
{
	tEdge  temp;
	int 	  i;
...
/*-------------------------------------------------------------------*/
void	ConvexHull3D::PrintFaces( void )
{
	int 	  i;
	tFace  temp;
...
the face.
-------------------------------------------------------------------*/
void	ConvexHull3D::CheckEndpts ( void )
{
	int 	   i;
	tFace   fstart;
	tEdge   e;
...
------------------------------------------------------------------*/
void   ConvexHull3D:: EdgeOrderOnFaces ( void ) {
	tFace f = faces;
	tEdge new_edge;
	int i,j;
