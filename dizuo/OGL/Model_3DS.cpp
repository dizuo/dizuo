...
void Model_3DS::Load(char *name)
{
	...
	for (int j = 0; j < numMaterials; j++)
	{
		if (Materials[j].textured == false)
		{
			...
			Materials[j].texID = CTextureManager::GetInstance()->BuildColorTexture(r, g, b);
			Materials[j].textured = true;
		}
	}
}

void Model_3DS::DrawModel()
{
	glPushMatrix();
		...
		for (int i = 0; i < numObjects; i++)
		{
			...
			for (int j = 0; j < Objects[i].numMatFaces; j ++)
			{
				...
				glBindTexture(GL_TEXTURE_2D, Materials[Objects[i].MatFaces[j].MatIndex].texID);
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	//	Change the texture file name from JPG to BMP
	for (size_t i=0; ; i++)
	{
		if(name[i] == '.')
		{
	...
	Materials[matindex].texID = LoadTexture(fullname);
