...
public:
	...
	{}
	CRectangle (const Vec2<T>& pos, const Vec2<T>& size)
	...
	Vec2<T> GetPos() const { return Vec2<T>(m_Left, m_Top); }
	Vec2<T> GetSize() const { return Vec2<T>(GetWidth(), GetHeight()); }
	Vec2<T> GetCenter() const { return Vec2<T>( (m_Left+m_Right)/2, (m_Top+m_Bottom)/2 );}
